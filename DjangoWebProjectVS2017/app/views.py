...
from app.forms import QuestionForm, ChoiceForm,UserForm, DropForm
...
def index(request):
    ...
    if request.method == "POST":
        ...
        if  form.is_valid():
            ...
            latest_question_list = Question.objects.order_by('-pub_date').filter(tema = q.tema)
            topic = q.tema

    else:
        latest_question_list = Question.objects.order_by('-pub_date')
    listOfTopics = Question.objects.values('tema').distinct()
...
def results(request, question_id, choice_id):
    ...
    notExistsCorrectAnswer = Choice.objects.filter(question = question, correct = True).count() == 0
    return render(request, 'polls/results.html', {'title':'Resultados de la pregunta:','question':question, 'choice':choice, 'notExistsCorrect':notExistsCorrectAnswer})
...
def choice_add(request, question_id):
        ...
        if request.method =='POST':
            ...
            if form.is_valid():
                ...
                existsCorrectAnswer = Choice.objects.filter(question = question, correct = True).count() == 1
        ...
        return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form, 'result': message})
